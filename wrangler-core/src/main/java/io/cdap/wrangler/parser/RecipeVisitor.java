...
/**
 * This class <code>RecipeVisitor</code> implements the visitor pattern used
 * during traversal of the AST tree. The <code>ParserTree#Walker</code> invokes
 * appropriate methods as call backs with information about the node.
 *
 * <p>
 ...
 * represents all the <code>TokenGroup</code> for all directives in a recipe.
 * Each directive will create a <code>TokenGroup</code></p>
 *
 * <p>
 ...
 * <code>TokenGroup</code> contains parsed <code>Tokens</code> for that
 * directive along with more information like <code>SourceInfo</code>. A
 * collection of <code>TokenGroup</code> consistutes a <code>RecipeSymbol</code>
...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
    ...
    // This is new methods added to the class.
    /**
     * A Recipe is made up of Directives and Directives is made up of each
     * individual Directive. This method is invoked on every visit to a new
    ...
    /**
     * A Directive can include identifiers, this method extracts that token that
     * is being identified as token of type <code>Identifier</code>.
     */
    @Override
    ...
    /**
     * A Directive can include properties (which are a collection of key and
     * value pairs), this method extracts that token that is being identified as
     * token of type <code>Properties</code>.
     */
    @Override
    ...
    /**
     * A Pragma is an instruction to the compiler to dynamically load the
     * directives being specified from the <code>DirectiveRegistry</code>. These
     * do not affect the data flow.
     *
     * <p>
     * E.g. <code>#pragma load-directives test1, test2, test3;</code> will
     * collect the tokens test1, test2 and test3 as dynamically loadable
     * directives.
     * <p>
     */
    @Override
    ...
    /**
     * A Pragma version is a informational directive to notify compiler about
     * the grammar that is should be using to parse the directives below.
     */
    @Override
     ...
     * start:end=value[,start:end=value]*. This visitor method allows you to
     * collect all the number ranges and create a token type
     * <code>Ranges</code>.
     */
    @Override
    ...
    /**
     * This visitor method extracts the custom directive name specified. The
     * custom directives are specified with a bang (!) at the start.
     */
    @Override
     ...
     * directive would operate on. When a token of type column is visited, it
     * would generate a token type of type <code>ColumnName</code>.
     */
    @Override
     ...
     * within a single-quote or a double-quote. This visitor method extracts the
     * string value within the quotes and creates a token type
     * <code>Text</code>.
     */
    @Override
    ...
    /**
     * A Directive can consist of numeric field. This visitor method extracts
     * the numeric value <code>Numeric</code>.
     */
    @Override
     ...
     * such a token type is found, the visitor extracts the expression and
     * generates a token type <code>Expression</code> to be added to the
     ...
     * token type <code>ColumnNameList</code> to be added to
     * <code>TokenGroup</code>.
     */
    @Override
     ...
     * token type <code>NumericList</code> to be added to
     * <code>TokenGroup</code>.
     */
    @Override
    ...
    public RecipeSymbol.Builder visitByteSize(DirectivesParser.ByteSizeContext ctx) {
        String text = ctx.ByteSizeLiteral().getText();
    ...
    public RecipeSymbol.Builder visitTimeDuration(DirectivesParser.TimeDurationContext ctx) {
        String text = ctx.TimeDurationLiteral().getText();
        builder.addToken(new TimeDuration(text));
        return builder;
    }

    /**
     * This visitor methods extracts the list of booleans specified. It creates
     * a token type <code>BoolList</code> to be added to
     * <code>TokenGroup</code>.
     */
    @Override
    ...
    public RecipeSymbol.Builder visitByteSizeArg(DirectivesParser.ByteSizeArgContext ctx) {
        String raw = ctx.getText();               // Get the full text like "5MB"
        builder.addToken(new ByteSize(raw));      // Create and add a ByteSize token
        return builder;
    }

    @Override
    public RecipeSymbol.Builder visitTimeDurationArg(DirectivesParser.TimeDurationArgContext ctx) {
        String raw = ctx.getText();                     // Get the full text like "30s"
        builder.addToken(new TimeDuration(raw));        // Create and add a TimeDuration token
        return builder;
    }

    /**
     * This visitor methods extracts the list of strings specified. It creates a
     * token type <code>StringList</code> to be added to
     * <code>TokenGroup</code>.
     */
    @Override
